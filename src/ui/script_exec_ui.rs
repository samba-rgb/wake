...
pub fn draw_script_execution_ui(f: &mut Frame, state: &ScriptExecutionUIState) {
    ...
    let progress = Gauge::default()
        .block(dark_block("Progress"))
        .gauge_style(Style::default().fg(Color::Green).bg(Color::DarkGray))
        .ratio(state.progress_percentage() as f64 / 100.0)
        .label(Span::styled(
            format!("{}/{} pods", state.completed_pods, state.total_pods),
    ...
    if let Some(selected) = state.get_selected_result() {
        let mut output_lines = vec![
            Line::from(vec![
                Span::styled("Pod: ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
                Span::styled(format!("{}/{}", selected.pod_namespace, selected.pod_name), Style::default().fg(Color::White)),
            ]),
            Line::from(vec![
                Span::styled("Exit Code: ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
                Span::styled(
                    selected.exit_code.to_string(),
            ...
            Line::from(""),
            Line::from(Span::styled("STDOUT:", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD))),
        ];

        for line in selected.stdout.lines().take(10) {
            output_lines.push(Line::from(Span::styled(line, Style::default().fg(Color::White))));
        }
        ...
        if !selected.stderr.is_empty() {
            ...
            for line in selected.stderr.lines().take(10) {
                output_lines.push(Line::from(Span::styled(line, Style::default().fg(Color::LightRed))));
            }
        }

        let output = Paragraph::new(output_lines)
            .block(dark_block("Output"))
            .style(dark_bg())
            .wrap(Wrap { trim: true });
